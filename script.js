...
document.getElementById("transferProduceBtn").onclick = async () => {
  ...
  const tx = await contract.transferProduce(
    produceId,
    newOwner,
    soldPrice,
    date
...
// function formatProduceData(data) {
//   let text = "";
//   text += "Produce Details:\n";
//   for (const [key, value] of Object.entries(data["Produce Details"])) {
//     text += `- ${key}: ${value}\n`;
//   }

//   text += "\nOrigin:\n";
//   for (const [key, value] of Object.entries(data.Origin)) {
//     text += `- ${key}: ${value}\n`;
//   }

//   text += "\nSupply Chain:\n";
//   data["Supply Chain"].forEach((agent, index) => {
//     text += `${index + 1}. Agent: ${agent.Agent}\n`;
//     text += `   - Bought At: ${agent["Bought At"]}\n`;
//     text += `   - Date: ${agent.Date}\n`;
//   });
